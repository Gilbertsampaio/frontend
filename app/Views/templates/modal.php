<div class="modal fade" id="" tabindex="-1" aria-labelledby="modalTitulo" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalTitulo"></h5>
        <i class="mdi mdi-window-close btn-close" data-bs-dismiss="modal" aria-label="Close"></i>
      </div>
      <div class="modal-body"></div>
      <div class="modal-footer"></div>
    </div>
  </div>
</div>

<div class="visibHid" id="divSearch">
  <div class="divClose">
    <i class="mdi mdi-window-close closeSearch"></i>
  </div>
  <div class="divSearch">
    <input autocomplete="off" type="text" class="search text-dark" name="localizar" id="localizar">
  </div>
</div>

<div class="fundSearch fade"></div>